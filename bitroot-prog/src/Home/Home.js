            ...
            <div className="header" style={{textAlign:"center"}}>
                {posts.map(post => (
                    <div id="contentcard" className="contentcard card" key={post.id}>
                        ...
                        <br />
                        <p className="card-footer text-muted bg-transparent border-white" style={{textAlign:"left"}}>{post.author.name} - {post.author.role}</p><p className="card-footer text-muted bg-transparent border-white" style={{textAlign:"right", marginTop:"-9.5vh"}}>{moment(post.date).date(18592).format("MMM D, YYYY")}</p>
                    </div>
                ))}
            </div>
